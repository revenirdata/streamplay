...
try {
  ...
  const run = await runScenario(JSON.parse(await readFile(path, 'utf8')), { store: createStore(config.dataDir), kafka: config.kafka, topology, adapters: await loadLocalAdapter(config.adapterModule), signal: controller.signal });
